...
const PartnerRegistrationForm: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setSubmitted(false);
      setFormData({ name: '', email: '', company: '', website: '', type: 'strategic', message: '' });
    }, 4000);
  };
  ...
  return (
    <div className="glass p-8 md:p-12 rounded-3xl border border-slate-200 shadow-2xl relative overflow-hidden">
      ...
      <h3 className="text-3xl font-bold text-slate-900 mb-2">{t('forms.partner.title')}</h3>
      <p className="text-slate-600 mb-8">{t('forms.partner.subtitle')}</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-2">{t('forms.partner.website')}</label>
            <input type="url" className="input-field" value={formData.website} onChange={e => setFormData({...formData, website: e.target.value})} />
          </div>
        </div>
        <div>
          <label className="block text-sm font-semibold text-slate-700 mb-2">{t('forms.partner.type')}</label>
          <select className="input-field" value={formData.type} onChange={e => setFormData({...formData, type: e.target.value})}>
            <option value="strategic">{t('forms.partner.types.strategic')}</option>
            <option value="financial">{t('forms.partner.types.financial')}</option>
            <option value="technology">{t('forms.partner.types.technology')}</option>
            <option value="academic">{t('forms.partner.types.academic')}</option>
          </select>
        </div>
        <div>
          <label className="block text-sm font-semibold text-slate-700 mb-2">{t('forms.partner.message')}</label>
          <textarea className="input-field h-32" value={formData.message} onChange={e => setFormData({...formData, message: e.target.value})}></textarea>
        </div>
